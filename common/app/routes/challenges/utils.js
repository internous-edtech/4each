...
export function getPreFile({ challengeType ,language}) {
  return {
    name: 'index',
    ext: getExtension({ challengeType ,language}),
    key: getFileKey({ challengeType, language })
  };
}

export function getExtension({ challengeType, language }) {
    let ext = ""

    if(challengeType == shakyo || challengeType == server){
        ext = language

        if (language == "php" || language == "java" || language == "perl" || language == "ruby"){
            ext = "server"
        }
...
export function getFileKey({ challengeType, language }) {
  return 'index' + (getExtension({ challengeType, language }) || 'html');
}
...
export function getPreviousChallenge(
  ...
  if (
    ...
    // if we find a next challenge and it is a coming soon
    // recur with plus one to skip this challenge
    return getPreviousChallenge(current, entities, { isDev, skip: skip - 1 });
  }
  return nextChallenge;
}

export function getPreviousChallengeOfNextBlock(
  current,
  entities,
  {
    isDev = false,
  ...
  // grab first challenge from next block
  const nextChallenge = challengeMap[newBlock.challenges[newBlock.challenges.length-1]];
  if (isDev || !nextChallenge || !nextChallenge.isComingSoon) {
    return nextChallenge;
  }
  // if first challenge is coming soon, find next challenge here
  ...
  // whole block is coming soon
  // skip this block
  return getPreviousChallengeOfNextBlock(
    current,
    entities,
    { isDev, skip: skip - 1 }
  );
...
export function getPreviousChallengeOfNextSuperBlock(
  current,
  entities,
  superBlocks,
  {
  ...
  const index = superBlocks.indexOf(superBlock.dashedName);
  const newSuperBlock = SuperBlockMap[superBlocks[ index +
     - 1 + skip]];
  if (!newSuperBlock) {
    return null;
  }
  ...
  // whole block is coming soon
  // grab first challenge in next block in newSuperBlock instead
  const challengeInNextBlock = getPreviousChallengeOfNextBlock(
    nextChallenge.dashedName,
    entities,
    { isDev }
  );
  ...
  // whole super block is coming soon
  // skip this super block
  return getPreviousChallengeOfNextSuperBlock(
    current,
    entities,
    superBlocks,
    { isDev, skip: skip - 1 }
